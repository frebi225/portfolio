  ...
  const generateResponse = (userInput: string): string => {
    ...
    const responses = [
      ...
      },
      {
        ...
        response: `${greeting}Lem-in est l'un des projets les plus complexes d'Audrey ! C'est un simulateur algorithmique qui fait déplacer des fourmis dans un réseau de salles de manière optimale. Le défi principal était de parser une carte en texte, construire un graphe, puis optimiser les chemins pour que toutes les fourmis arrivent à destination en un minimum de tours. Audrey a utilisé Go pour implémenter des algorithmes de recherche de chemins (BFS/DFS) et d'optimisation de flux. Le projet gère des cas complexes comme les cycles, la répartition intelligente des fourmis entre plusieurs chemins, et l'affichage précis de la simulation tour par tour.`,
      },
      {
        keywords: [
          "orivie",
        ...
        response: `${greeting}Orivie est l'un des projets les plus ambitieux d'Audrey ! C'est une application mobile e-santé révolutionnaire pour les patients ivoiriens qui permet un accès sécurisé au dossier médical numérique partout en Côte d'Ivoire, même en zone rurale. L'app utilise le numéro CMU comme identifiant unique et propose des fonctionnalités innovantes comme un chatbot IA santé, la géolocalisation des centres de santé, la Météo Santé avec alertes sanitaires, et l'intégration des données publiques ivoiriennes. Le projet vise à réduire les pertes d'antécédents médicaux et accélérer la prise en charge d'urgence.`,
      },
      {
        keywords: [
          "sécurité",
        ...
        response: `${greeting}Audrey a suivi une formation spécialisée en sécurité numérique avec We.Code, où elle a appris les bonnes pratiques de sécurisation sous Linux, l'utilisation d'outils comme Wireshark pour l'analyse réseau, et la gestion des pare-feux et permissions. Cette formation lui a donné des compétences solides en cybersécurité et protection des systèmes.`,
      },
      {
        keywords: [
        ...
        response: `${greeting}Audrey a récemment suivi une formation intensive aux outils d'IA avec Femmes Connectées. Elle a appris à utiliser des outils d'intelligence artificielle pour améliorer la productivité et l'automatisation, ainsi que des solutions no-code pour la génération d'idées innovantes. Cette formation lui permet d'intégrer l'IA dans ses projets de développement.`,
      },
    ]
